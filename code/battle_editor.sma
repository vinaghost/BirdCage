...
public tele(id)
{
	new Float:origin[3], origin_2[3];
	
	
	if(SsGetOrigin(origin))
	{
		for (new i = 0; i < 3; i++)
			origin_2[i] = floatround(origin[i]);
		set_user_origin(id, origin_2  );
		client_print(id, print_chat, "TELE thanh cong");
	}
	else
	{
...
readSpawns() {
	...
	format(MapFile, 255, "%s/bc/spawns/%s_battle.cfg",config, Map);
	g_TotalSpawns = 0;
...
add_spawn(vecs[3], angles[3], vangles[3]) {
	...
	get_mapname(Map, 31);
	get_configsdir(config, 31);
	format(MapFile, 255, "%s\bc\spawns\%_battle.cfg",config, Map);
